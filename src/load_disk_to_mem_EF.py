...
fd_1, err_1, size_1, atime_1, mtime_1, ctime_1 = list_all_files(src_6, fd, err, size, atime, mtime, ctime)
fd, err, size, atime, mtime, ctime = create() # reset
fd_2, err_2, size_2, atime_2, mtime_2, ctime_2 = list_all_files(src_7, fd, err, size, atime, mtime, ctime)
fd, err, size, atime, mtime, ctime = create() # reset
...
total_1, used_1, free_1 = disk_usage(src_6)
total_2, used_2, free_2 = disk_usage(src_7)
...
sentence_1 = [
    ...
    "Total file size in " + src_6 + " (byte): free/used/used(accessible)/total: " + str(free_1) + "/" + str(used_1) + "/" + str(tfs_1) + "/" + str(total_1) + "\n",
    "Total file size in " + src_7 + " (byte): free/used/used(accessible)/total: " + str(free_2) + "/" + str(used_2) + "/" + str(tfs_2) + "/" + str(total_2) + "\n",
    "Total file size (GB): free/used/used(accessible)/total: " + str(free//(2**30)) + "/" + str(used//(2**30)) + "/" + str(tfs//(2**30)) + "/" + str(total//(2**30)) + "\n",
    "Total file size in " + src_6 + " (GB): free/used/used(accessible)/total: " + str(free_1//(2**30)) + "/" + str(used_1//(2**30)) + "/" + str(tfs_1//(2**30)) + "/" + str(total_1//(2**30)) + "\n",
    "Total file size in " + src_7 + " (GB): free/used/used(accessible)/total: " + str(free_2//(2**30)) + "/" + str(used_2//(2**30)) + "/" + str(tfs_2//(2**30)) + "/" + str(total_2//(2**30)) + "\n",
    "Total number of accessible files: " + str(len(fd_1) + len(fd_2)) + "\n",
    "Total number of unaccessible files: " + str(len(err_1) + len(err_2)) + "\n",
    "Total number of accessible files in " + src_6 + " : " + str(len(fd_1)) + "\n",
    "Total number of accessible files in " + src_7 + " : " + str(len(fd_2)) + "\n",
    "Total number of inaccessible files in " + src_6 + " : " + str(len(err_1)) + "\n",
    "Total number of inaccessible files in " + src_7 + " : " + str(len(err_2)) + "\n"
]
sentence_2 = [
    "\n\n",
    "All accessible files in " + src_6 + ": (directory | size(byte) | atime | mtime | ctime)\n\n",
]
sentence_3 = [
    "\n\n",
    "All accessible files in " + src_7 + ": (directory | size(byte) | atime | mtime | ctime)\n\n",
]
sentence_4 = [
    "\n\n",
    "All inaccessible files in " + src_6 + ":\n\n",
]
sentence_5 = [
    "\n\n",
    "All inaccessible files in " + src_7 + ":\n\n",
]

# Write log
f = open('./src/log/all_accessible_files_EF.txt', 'w', encoding='utf-8')
